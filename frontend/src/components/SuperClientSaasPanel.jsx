...
export default function SuperClientSaasPanel({ token, client, onBack }) {
  ...
  async function loadAll() {
    ...
    try {
      const [branchRes, userRes] = await Promise.all([
        api(token).get("/api/role-access/super/tenants/" + setupClientId + "/branches"),
        api(token).get("/api/role-access/super/tenants/" + setupClientId + "/users")
      ]);
  ...
  async function createBranch(event) {
    ...
    try {
      await api(token).post("/api/role-access/super/tenants/" + setupClientId + "/branches", branchForm);
  ...
  async function createLogin(event) {
    ...
    try {
      await api(token).post("/api/role-access/super/tenants/" + setupClientId + "/users", {
        ...userForm,
  ...
  async function toggleUser(user) {
    try {
      await api(token).patch("/api/role-access/super/tenants/" + setupClientId + "/users/" + user.id, {
