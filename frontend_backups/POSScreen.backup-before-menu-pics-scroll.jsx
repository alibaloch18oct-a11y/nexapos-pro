...
const demoItems = demoCatalog.flatMap((group, groupIndex) =>
  group.products.map((entry, index) => ({
    ...
    isActive: true,
    isAvailable: true,
    image: createImageCard(entry[0], entry[1], entry[3], group.colors[0], group.colors[1])
  }))
);
    ...
    <div className="pos-modal-backdrop">
      <div className={`pos-modal-card ${wide ? "wide" : ""}`}>
        <div className="pos-modal-head">
          ...
          <button className="pos-ghost-btn" onClick={onClose}>
            ✕
          </button>
        </div>
        {children}
...
export default function POSScreen({ token, module, session, onBack }) {
  ...
  async function loadMenu() {
    try {
      ...
      const finalCategories = apiCategories.length ? apiCategories : demoCategories;
      const finalItems = attachImages(apiItems.length ? apiItems : demoItems);

      setCategories(finalCategories);
      setMenuItems(finalItems);

      if (!activeCategoryId && finalCategories.length) {
        setActiveCategoryId(finalCategories[0].id);
      }
    } catch {
      setCategories(demoCategories);
      setMenuItems(attachImages(demoItems));
      if (!activeCategoryId) setActiveCategoryId(demoCategories[0].id);
    }
  }
